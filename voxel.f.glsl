...
void main()
{
    vec3 outside = sh_light(pass_normal, beach);
    vec3 inside = sh_light(pass_normal, groove)*0.004;
    vec3 ambient = mix(inside, outside, pass_light);
    
    out_color = vec4(ambient * pass_color, 1.0);
}